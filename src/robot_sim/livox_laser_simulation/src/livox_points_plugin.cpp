...
void convertDataToRotateInfo(const std::vector<std::vector<double>> &datas, std::vector<AviaRotateInfo> &avia_infos) {
    ...
    for (auto &data : datas) {
        if (data.size() == 3) {
            ...
            avia_infos.back().zenith = data[2] * deg_2_rad;  //转化成标准的右手系角度
        } else {
...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        pcl::PointCloud<pcl::PointXYZI>::Ptr simu_cloud(new pcl::PointCloud<pcl::PointXYZI>);
        ...
        for (auto &pair : points_pair) {
            ...
            //     auto index = verticle_index * rayCount + horizon_index;
        ...
        pcl::toROSMsg(*simu_cloud, scan_point2);
        scan_point2.header.stamp = ros::Time::now();
        scan_point2.header.frame_id = raySensor->Name();
